...
int main() 
{
    ...
    for (index =0; index < NUM_TEST_DISKS; index++) 
    {
        ...
        diskName[4] = '0'+index;
        disks[index] = DiskManager::openDisk(diskName,0);
        
        if (disks[index] < 0)
        { 
            printf("] Open failed with (%i). Disk probably does not exist.\n",disks[index]);
            
	    disks[index] = DiskManager::openDisk(diskName,BLOCKSIZE * NUM_BLOCKS); /* create the disk */
	    if (disks[index] < 0)
            {
		...
		exit(0); 
            ...
            {
                retValue = DiskManager::writeBlock(disks[index],testBlocks[index2],buffer);
                if (retValue < 0)
		{
 ...
 * Check every single byte */
            for (index2 = 0; index2 < NUM_TEST_BLOCKS; index2++)
            {
		if (DiskManager::readBlock(disks[index],testBlocks[index2],buffer) < 0)
                {
